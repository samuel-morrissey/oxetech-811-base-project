import { PrismaClient } from "@prisma/client";
import { hashPassword } from "../src/services/security/password";

const prisma = new PrismaClient();

const PASSWORD_ANA = hashPassword(process.env.SEED_PASSWORD_ANA || "dev-ana-local-pass-123");
const PASSWORD_BRUNO = hashPassword(process.env.SEED_PASSWORD_BRUNO || "dev-bruno-local-pass-123");
const PASSWORD_CARLA = hashPassword(process.env.SEED_PASSWORD_CARLA || "dev-carla-local-pass-123");

const users = [
  {
    id: "user_ana",
    name: "Ana Beatriz",
    email: "ana.aluna@example.com",
    role: "student",
    password: PASSWORD_ANA,
  },
  {
    id: "user_bruno",
    name: "Bruno Lima",
    email: "bruno.professor@example.com",
    role: "teacher",
    password: PASSWORD_BRUNO,
  },
  {
    id: "user_carla",
    name: "Carla Suporte",
    email: "carla.suporte@example.com",
    role: "support",
    password: PASSWORD_CARLA,
  },
];

const tickets = [
  {
    id: "ticket_001",
    title: "Nao consigo acessar o ambiente virtual",
    description: "A pagina de login informa erro mesmo com a senha correta.",
    category: "sistemas",
    status: "open",
    priority: "high",
    requesterId: "user_ana",
    assignedToId: "user_carla",
    createdAt: new Date("2026-05-01T10:00:00.000Z"),
    updatedAt: new Date("2026-05-01T10:00:00.000Z"),
  },
  {
    id: "ticket_002",
    title: "Projetor da sala 204 nao liga",
    description: "O equipamento esta sem imagem e a aula comeca em uma hora. Urgente.",
    category: "infra",
    status: "in_progress",
    priority: "urgent",
    requesterId: "user_bruno",
    assignedToId: "user_carla",
    createdAt: new Date("2026-05-02T08:15:00.000Z"),
    updatedAt: new Date("2026-05-02T08:35:00.000Z"),
  },
  {
    id: "ticket_003",
    title: "Duvida sobre prazo de atividade",
    description: "Preciso confirmar a data final de entrega do projeto integrador.",
    category: "academico",
    status: "resolved",
    priority: "medium",
    requesterId: "user_ana",
    assignedToId: "user_bruno",
    createdAt: new Date("2026-05-03T14:20:00.000Z"),
    updatedAt: new Date("2026-05-04T09:00:00.000Z"),
  },
];

const comments = [
  {
    id: "comment_001",
    ticketId: "ticket_002",
    authorId: "user_carla",
    message: "Chamado recebido. Vou verificar o equipamento antes da aula.",
    createdAt: new Date("2026-05-02T08:35:00.000Z"),
  },
  {
    id: "comment_002",
    ticketId: "ticket_003",
    authorId: "user_bruno",
    message: "O prazo final esta registrado no ambiente virtual da disciplina.",
    createdAt: new Date("2026-05-04T09:00:00.000Z"),
  },
  {
    id: "comment_003",
    ticketId: "ticket_001",
    authorId: "user_ana",
    message: "Continuo sem acesso mesmo apos redefinir a senha.",
    createdAt: new Date("2026-05-01T11:30:00.000Z"),
  },
];

async function main() {
  // Limpa na ordem inversa para respeitar foreign keys
  await prisma.ticketComment.deleteMany();
  await prisma.ticket.deleteMany();
  await prisma.user.deleteMany();

  // Cria os dados
  for (const user of users) {
    await prisma.user.create({ data: user });
  }

  for (const ticket of tickets) {
    await prisma.ticket.create({ data: ticket });
  }

  for (const comment of comments) {
    await prisma.ticketComment.create({ data: comment });
  }

  console.log("Banco de dados populado com sucesso.");
}

main()
  .catch((error) => {
    console.error(error);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
